...
module.exports = inherit(CronRunner, {
    __constructor: function (options) {
        ...
        if (!o.lifetime) {
            console.warn('Snapshot "lifetime" option was not set. Default snapshot "lifetime" will be set');
    ...
    _onError: function (error) {
        console.error('Error occur while clean old snapshots');
        console.error(error.message);
        throw error;
    },

    execute: function () {
        console.info('-- start execute snapshot cleaner --');
        ...
        return vow.all([vowNode.promisify(fs.readdir)(snapshotsPath), vow.allResolved(symlinkRealPaths)])
            ...
            .then(function (snapshots) {
                ...
                snapshots = _.chain(snapshots)
                    ...
                    .map(function (item) {
                        console.info('Snapshot with name %s will be removed', item);
            ...
            .then(function () {
                console.info('-- end execute snapshot cleaner -- ');
            })
            .fail(this._onError);
    }
}, {
